...
class Header extends Component {
  ...
  render() {
    const { title, titleClick, leftIcon, leftTitle1, leftClick1, leftClick2, leftTitle2, rightIcon, rightTitle, rightClick, searchTitle, onSearch, noLine, cancelText, autoFocusInst, onSearchFocus } = this.props
    return <div className={classnames(style.header, this.props.className)}>
      ...
      >{searchTitle ? <SearchBar onFocus={onSearchFocus} ref={ (ref) => { autoFocusInst ? this.autoFocusInst = ref : null }} cancelText={cancelText} key={6} onSubmit={onSearch} className='search' placeholder={searchTitle} maxLength={8}/> : <span key={7} onClick={titleClick} className='title'>{title}
        </span>}</NavBar>
