...
class Header extends Component {
  constructor(props) {
    super(props)
    this.state = {}
    // this.autoFocusInst = this.autoFocusInst.bind(this)
  }
  componentDidMount() {
  ...
  // }
  render() {
    const { title, titleClick, leftIcon, leftTitle1, leftClick1, leftClick2, leftTitle2, rightIcon, rightTitle, rightClick, searchTitle, onSearch, noLine, cancelText } = this.props
    return <div className={classnames(style.header, this.props.className)}>
      ...
      >{searchTitle ? <SearchBar ref={ (ref) => { this.props.autoFocusInst ? this.autoFocusInst = ref : null }} cancelText={cancelText} key={6} onSubmit={onSearch} className='search' placeholder={searchTitle} maxLength={8}/> : <span key={7} onClick={titleClick} className='title'>{title}
        </span>}</NavBar>
